  ...
  integrations: [
    ...
    AstroPWA({
      ...
      workbox: {
        ...
        // The "Compartir PDF" chunk graph (nota-pdf + jspdf + html2canvas +
        // dompurify) is reached only from a dynamic import off the checkout-
        // success button — precaching it blocks every first SW install.
        // Fetched once on first real use, then CacheFirst below. informe-pdf.ts
        // was removed (informes now exports .xlsx, no PDF/print) so nota-pdf.ts
        // is the ONLY jspdf importer again — jspdf.es.min may re-inline into
        // nota-pdf.*.js, in which case this glob simply matches nothing
        // (harmless); it stays in case the bundler ever re-splits it.
        globIgnores: [
          '**/_astro/nota-pdf.*.js',
        ...
        runtimeCaching: [
          {
            ...
            urlPattern: /^\/_astro\/(nota-pdf|jspdf\.es\.min|html2canvas\.esm|purify\.es)\.[^/]+\.js$/,
            handler: 'CacheFirst',
